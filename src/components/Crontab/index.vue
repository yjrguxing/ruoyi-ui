<template>
    <div>
        <el-tabs type="border-card">
            ...
            <el-tab-pane label="日時" v-if="shouldHide('hour')">
                <CrontabHour
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
...
const tabTitles = ref(["秒", "分", "日時", "日", "月", "週", "年"])
...
// サブコンポーネントによってトリガーされ、式組成のフィールド値を変更する
function updateCrontabValue(name, value, from) {
    crontabValueObj.value[name] = value
}
// ティーブル単一オプションのサブコンポーネント検査証デジタルフォーマット（-props転送）
